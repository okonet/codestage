...
describe('resolveStylesheetsDir', () => {
  it('should resolve path to highlight.js/styles dir', () => {
    expect(resolveStylesheetsDir()).toEqual(
...
describe('loadStylesheet', () => {
  it('should load highlight CSS and return a string', () => {
    const sheet = loadStylesheet('agate')
...
`
    ...
    expect(matchDeclarations(ast, 'hljs-comment').length).toEqual(1)
    expect(matchDeclarations(ast, 'hljs-keyword').length).toEqual(2)
    expect(matchDeclarations(ast, 'hljs-quote').length).toEqual(2)
    // Proper inheritance
    expect(csstree.translate(matchDeclarations(ast, 'hljs-quote')[0])).toEqual('color:#aa0d91')
    expect(csstree.translate(matchDeclarations(ast, 'hljs-quote')[1])).toEqual('background:#fff')
  })
})

describe('getSheet', () => {
  it('should return a AST object', () => {
    const res = getSheet(loadStylesheet('agate'))
    expect(res).toMatchSnapshot()
  })
})
