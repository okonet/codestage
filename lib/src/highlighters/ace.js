  ...
  (acequire, exports, module) => {
    ...
    function renderSync(input, mode, lineStart = 1, disableGutter = true) {
      ...
      for (let idx = 0; idx < length; idx += 1) {
        ...
        stringBuilder.push(`</div>${idx < length - 1 ? '\n' : ''}`)
      }
...
async function loadTheme(themeName) {
  const { cssClass, cssText } = await requireAceModule(`ace/theme/${themeName}`)
  // Remove specificity
  const re = new RegExp(`.${cssClass} .ace`, 'g')
  let cssTextStripped = cssText.replace(re, '.ace')
  // Replace theme selector with a more generic one so we don't have to pass theme name to renderer
  const re2 = new RegExp(`.${cssClass}`, 'g')
  cssTextStripped = cssTextStripped.replace(re2, '.ace_static_highlight')
  return {
    name: themeName,
...
async function highlight(value, language) {
