global.window = {} // Ace needs the global window object :shrug:
...
ace.define(
  ...
  (acequire, exports, module) => {
    ...
    function renderSync(input, mode, theme, lineStart = 1, disableGutter = true) {
      ...
      const html = `<div class='ace-${theme} ace_static_highlight${disableGutter
        ? ''
        : ' ace_show_gutter'}' style='counter-reset:ace_line 
${lineStart - 1}'>${stringBuilder.join('')}</div>`
      textLayer.destroy()
      return html
    }

    function renderToStaticMarkup(input, language, theme) {
      return renderSync(input, language, theme.name)
    }
...
async function loadTheme(themeName) {
  ...
  const re = new RegExp(`.ace-${themeName} .ace`, 'g')
  const cssTextStripped = cssText.replace(re, '.ace')
  return {
    name: themeName,
    cssText: cssTextStripped
  }
}
...
async function highlight(value, language, { theme }) {
  const themeObject = await loadTheme(theme)
  // eslint-disable-next-line
  require(`brace/mode/${language}`)
  return ace.acequire(['static_highlight'], module => module(value, language, themeObject))
}
