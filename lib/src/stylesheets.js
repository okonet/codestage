...
function resolveStylesheetsDir(packageName, dirName) {
  const hlPath = path.dirname(require.resolve(`${packageName}/package.json`))
  return path.resolve(hlPath, dirName)
}

function loadStylesheet(themeName, packageName, dirName) {
  return fs.readFileSync(
    path.join(resolveStylesheetsDir(packageName, dirName), `${themeName}.css`),
    'utf-8'
  )
}
