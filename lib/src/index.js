...
const Highlighter = require('./highlighters/highlight-js')
const {
  loadStylesheet,
  resolvePackageDir,
  getSheet,
...
async function html2rtf(html, options) {
  ...
  const { theme } = options
  const sheet = getSheet(await loadStylesheet(theme, 'highlight.js', 'styles'))
...
async function highlight(value, language, options) {
  const highlightedHtml = Highlighter.highlight(value, language)
