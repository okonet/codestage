...
function renderRTF(htmlText, cssText, fontface = 'Courier New', fontsize = 40) {
  const colorTable = getUniqueColors(cssText)
...
async function highlight(value, language, options) {
  ...
  // eslint-disable-next-line no-param-reassign
  const { theme, fontface, fontsize, includeBackground } = options
  const { cssText, cssClass } = await Highlighter.loadTheme(theme).catch(error => {
    throw new Error(`Could not load theme ${theme}
    ${error}
    `)
  })

  const safeCssText = getSafeCssText(
    includeBackground ? cssText : stripBackgroundForSelector(cssText, cssClass)
  )
  const highlightedHtml = await Highlighter.highlight(value, language, options).catch(error => {
    throw new Error(`Could not highlight code using ${language}
    Code:
    ${value}
  ...
  const html = `<div class="${cssClass}">${highlightedHtml}</div>`
  return {
    html,
    value: renderRTF(html, safeCssText, fontface, fontsize)
  }
}
