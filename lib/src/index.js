...
function renderRTF(ast, options) {
  ...
  const { fontface, fontsize, theme } = options
  const sheet = getSheet(loadStylesheet(theme, 'prismjs', 'themes'))
...
\\f0\\fs${fontsize}
${all(wrappedAst, sheet)}
}`
}

/* Highlight `value` as `language`. */
function highlight(language, value, options) {
  const outputOfPrism = Prism.highlight(value, Prism.languages[language], language)
  const processor = unified().use(rehypeParse, {
    fragment: true
  })
  const ast = processor.parse(outputOfPrism)
  return {
    ast,
    value: renderRTF(ast.children, options)
  }
}
...
function highlightAuto(value, options) {
  console.log(options)
  const result = lowlight.highlightAuto.call(this, value, options)
  const { language } = result
  // result.value = renderRTF(result.value, options)
  // if (result.secondBest) {
  //   result.secondBest.value = renderRTF(result.secondBest.value, options)
  // }
  // return result
